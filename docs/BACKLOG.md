...
| D2 | Census realism check: personas answer held-out factual questions (home ownership, commute mode, vehicles, internet access…) and are scored against real ACS PUMS answers for their demographic cells; variables used to build personas are excluded | Proposed, awaiting approval | SPEC §8, TEST_PLAN S13, `tools/build-populations` (the same microdata already feeds the US table) |
