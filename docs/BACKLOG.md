...
| B24 | Code-signing certificate and the `WINDOWS_CERTIFICATE` secrets | **Workflow ready:** non-tag checks may build unsigned, but version tags now require the secrets; Tauri signs the application before bundling and CI verifies the executable plus both installers. Waiting on a trusted certificate and the two repository secrets | A tagged release passes `signature_valid` with the trusted certificate |
