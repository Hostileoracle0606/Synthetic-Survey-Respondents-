...
| B26 | Provision the Windows VMs (`win11-clean`, `win11-perf`, and `win10-clean` while Windows 10 is a target), register them as runners and set `RELEASE_VMS=true` | **Jobs ready:** tagged releases select all three runner labels; Windows 10 enforces a no-WebView2 starting snapshot and Windows 11 performance enforces FPS. Still needs cloud access, clean snapshots, runner registration and budget | The VM jobs in `release.yml` run on every tag |
...
| B28 | Remaining S14/S8 release tests: `upgrade_keeps_data`, `clean_install_win10_no_webview2`, `network_egress` | **Workflow ready:** upgrade/reinstall preservation and live process-level egress run on `windows-latest`; the Windows 10 bootstrap test is wired to `win10-clean`. Waiting on `GEMINI_API_KEY`, a first previous release for true vN→vN+1 coverage, and B26 for the Windows 10 run | Automated in the release jobs |
