...
| ~~B12~~ | ~~Cost: estimate before a run (`est_cost_usd`) and live cost in Step 4~~ **Done:** `estimate_run` shows calls and cost beside Run Survey Simulation (input tokens from the real prompts, output from earlier runs on the model when there are any) and is stored on the run; `RunProgress.costUsd` and the Step 4 card show actual against estimated. Prices come from the new price table (`survey_core::pricing`, `get_prices`/`set_price`), which covers only the price part of B1 | Needs the price table from the Settings screen (B1); the card shows "$—" until then | The estimate shows before Run Survey Simulation and cost updates live |
