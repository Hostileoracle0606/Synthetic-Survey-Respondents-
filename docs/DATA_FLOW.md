...
| API cost so far | Sum over this run's `llm_calls` of uncached input × input price + cached input × cached price + output × output price, using the price table in `settings` (`price_table`; a model with no saved price uses its tier's starting price). Shown as "$—" only for a model with neither. The card also shows the estimate stored on the run |
