...
- **Item 4 and 7, cost (B12).** `estimate_run` before the run (stored as `est_cost_usd`) and the live cost in `RunProgress`, from the price table (`survey_core::pricing`, the price part of B1).
