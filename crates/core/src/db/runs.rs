...
use crate::llm::Usage;
use crate::model::{
    CohortStatus, CostEstimate, Question, QuestionType, RespondentDetail, RunConfig, RunStatus,
    SimulationRun,
};
...
pub fn get(conn: &Connection, id: i64) -> AppResult<SimulationRun> {
    ...
    let cost_usd = pricing::price_for(conn, &row.4)?
...
pub fn estimate(conn: &Connection, project_id: i64, model: &str) -> AppResult<CostEstimate> {
    ...
    let cost_usd = pricing::price_for(conn, model)?.map(|p| {
        pricing::cost(
            &p,
            &Usage {
...
pub struct RunPlan {
    ...
    pub answered: u32,
    /// For the live cost; None when the model has no price.
...
pub fn plan(conn: &Connection, run_id: i64) -> AppResult<RunPlan> {
    ...
    Ok(RunPlan {
        ...
        price: pricing::price_for(conn, &run.model)?,
